...
class StockExchange(gym.Env):
    ...
    def __init__(self, seed=None):
        ...
        self.observation_space = spaces.Box(-1.0, 10.0, (self.num_state_space, self.ticker_length), dtype=np.float)
    ...
    def get_running_state(self):
        return np.zeros((self.num_state_space, self.ticker_length))# .tolist()
    ...
    def add_new_state(self, new_states_to_add):
        ...
        # Maybe just get rid of that all together?
        new_states = np.array([state[:-1].tolist() for state in new_states_to_add])
